...
//#endif
  ...
  if (!vm.count("simulation") && vm["debug.vision"].as<bool>()) {
   ...
   ThreadManager gameController("GameController", 250'000); // every quarter second
   ThreadManager offnaoTransmitter("OffnaoTransmitter", 50'000); // 20fps limit
   ...
   ThreadManager teamTransmitter("TeamTransmitter", 250'000); // every quarter second
   ThreadManager teamReceiver("TeamReceiver", 100'000); // 10fps limit (Congested WiFi: Higher than TeamTransmitter)
   ThreadManager remoteControlReceiver("RemoteControlReceiver", 200'000); // 5 fps limit for remote-control updates
