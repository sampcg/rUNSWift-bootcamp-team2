...
void TeamReceiver::naoHandler(const boost::system::error_code &ec,
                              std::size_t size) {
   if (Thread::name == NULL) {
   ...
   if (ec) {
      llog(ERROR) << "could not receive team packet: " << ec.message();
      static time_t lastsaid = 0;
      time_t now = time(NULL);
      if (now >= lastsaid + 5) {
         SAY("team receiver " + ec.message());
         lastsaid = now;
      }
   }
   ...
   if (size == sizeof(SPLStandardMessage)) {
      if (m->playerNum >= 1 && m->playerNum <= ROBOTS_PER_TEAM &&
        ...
        llog(INFO) << "Recieved packet from " << std::to_string(m->playerNum) << endl;
         llog(INFO) << "    State: " << std::to_string(bd->gameState) << endl;
      }
   } else {
...
void TeamReceiver::stdoutHandler(const boost::system::error_code &error,
   ...
   BroadcastData* bd = (BroadcastData*)m->data;
   llog(INFO) << "Received data from player from stdout " << std::to_string(m->playerNum) << endl;
