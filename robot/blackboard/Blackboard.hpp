...
#include "types/RefereeHandDetectorParams.hpp"
...
struct VisionBlackboard {
   ...
   RefereeHandDetectorSettings refereeHandDetectorSettings;
   RefereeHandDetectorParams refereeHandDetectorParams;
   bool refereeSeen;
};
...
class Blackboard : ProtobufSerialisable {
    ...
    public:
        ...
        /**
         * A copy of the [debug] `mask=<value>` from the config files, intended to be used to dump to a local file
         * without interference from other things, such as `OffNaoTransmitter::offnao_session::deliver` / `sendingMask`.
         */
        OffNaoMask_t debugMask;
