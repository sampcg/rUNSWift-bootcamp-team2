   ...
   while (!attemptingShutdown) {
      try {
         ...
         if (!setjmp(*jumpPoints[threadID])) {
            T t(bb);
            Timer timer;
            int32_t elapsedMicroseconds = 0;
            while (!attemptingShutdown) {
               timer.restart();

               if (padMicroseconds != TICK_NEVER) {
                  ...
                  elapsedMicroseconds = timer.elapsed_us();
                  llog(DEBUG) << "Thread '" << name << "' took "
                             << elapsedMicroseconds << " us." << std::endl;
                  if (elapsedMicroseconds < padMicroseconds) {
                     if (elapsedMicroseconds >= 0) {
                        usleep(padMicroseconds - elapsedMicroseconds);
                     } else
                        llog(ERROR) << "ERROR: Thread " << name << " took "
                                    << elapsedMicroseconds / 1000 << "ms!" << std::endl;
                  } else if (padMicroseconds > TICK_AS_FAST_AS_POSSIBLE || ((name == "Perception") && (elapsedMicroseconds >= 50000))){
                     llog(ERROR) << "WARNING: Thread " << name << " ran overtime: "
                                 << elapsedMicroseconds / 1000  << "ms!" << std::endl;
                     if (elapsedMicroseconds >= 1000000 && name == "perception")
                        SAY("perception overtime");
                  }
               } else {
                  sleep(10);  // thread does not need a 'tick'
                  llog(ERROR) << "WARNING: Thread " << name << " is using TICK_NEVER, which is deprecated and may be removed at any time!\n";
               }
            }
         }
