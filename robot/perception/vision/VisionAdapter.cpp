...
void VisionAdapter::tickProcess() {
    ...
    info_in.refereeHandDetectorSettings = readFrom(vision, refereeHandDetectorSettings);
    ...
    writeTo (vision, redRegions,      info_out.redRegions     );
    writeTo (vision, refereeHands,    info_out.refereeHands   );
    writeTo (vision, cropRegion,      info_out.cropRegion     );

    if (info_out.refereeHands.size() > 0) {
        writeTo (vision, refereeSeen, true);
    }
