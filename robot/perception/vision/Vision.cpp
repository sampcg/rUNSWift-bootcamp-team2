...
#include "perception/vision/detector/RefereeHandsDetector.hpp"
#include "soccer.hpp"
...
void Vision::setupAlgorithms_() {
    ...
    addDetector_(DETECTOR_REFEREE_HANDS, new RefereeHandsDetector());
#ifndef CTC_2_1
...
void Vision::runAlgorithms_() {
    ...
    t.restart();
    runDetector_(DETECTOR_REFEREE_HANDS);
    refereeHandsDetectorTime += t.elapsed_us();
    t.restart();
...
VisionInfoOut Vision::processFrame(const CombinedFrame& this_frame, const VisionInfoIn& info_in) {
    ...
    if(frameCount == 1000)
    {
        ...
        // Print the timings.
        llog(DEBUG) << std::endl << "VISION TIMINGS (us)" << std::endl;
        llog(DEBUG) << "Average fovea generation time: " <<
                                        ((float)foveaTime)/1000.0f << std::endl;
        llog(DEBUG) << "Average robot detector time: " <<
                                ((float)robotDetectorTime)/1000.0f << std::endl;
        llog(DEBUG) << "Average field features time: " <<
                                ((float)fieldFeaturesTime)/1000.0f << std::endl;
        llog(DEBUG) << "Average Region finder time: " <<
                                 ((float)regionFinderTime)/1000.0f << std::endl;
        llog(DEBUG) << "Average ball detector time: " <<
                               ((float)ballDetectorTime) / 1000.0f << std::endl;
        llog(DEBUG) << "Average referee hands detector time: " <<
                               ((float)refereeHandsDetectorTime) / 1000.0f << std::endl;
        llog(DEBUG) << "Average total processFrame time: " << ((float)(DCCTime+
