...
void PythonSkill::startInotify() {
   llog(INFO) << "Attempting to mount file watchers..." << endl;
   inotify_fd = inotify_init();

   const auto registerListener= [&](const std::string& watchPath) {
      int wd = inotify_add_watch(inotify_fd, watchPath.c_str(),
                             IN_MODIFY | IN_ATTRIB | IN_MOVED_FROM | IN_MOVED_TO | IN_DELETE);
      if (wd < 0) {
         llog(ERROR) << "Failed to start watching directory: " << watchPath << endl;
      } else {
         llog(INFO) << "Watching directory: " << watchPath << endl;
      }
   };
   registerListener(path);
   registerListener(std::string(path) + "body/skills");
   registerListener(std::string(path) + "head");
...
void PythonSkill::startPython() {
   ...
   try { 
      ...
      object exceptions_module = import("builtins");
...
BehaviourRequest PythonSkill::execute() {
   ...
   try {
      ...
      if (reloadNeeded || errorOccured) {
         if(reloadNeeded){
            llog(INFO) << "Changes detected! Reloading Python..." << endl;
            std::cout << "Changes detected! Reloading Python..." << endl;
         }
         reloadPython();
         errorOccured = false;
      } else if (! errorOccured) {
         // This creates a C++ Blackboard instance... :(
         ...
         while (i < len) {
            ...
            if (event->len) {
               ...
               // if (regex_match(event->name, matchRegex)) {
                  ...
                  std::cout << "Detected change in " << event->name << endl;
                  reloadNeeded = true;
                  break;
               // }
            }
