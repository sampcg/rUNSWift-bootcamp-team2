...
cv::Rect RefereeHandsDetector::BBoxToRect(const BBox& box) {
   cv::Rect r;
   r.x = box.a.x();
   r.y = box.a.y();
   r.width = box.width();
...
//       return;
//    }
//    cv::Mat src = yuyvToRgbMat(info_in.top_frame, TOP_IMAGE_COLS, TOP_IMAGE_ROWS);
...
cv::Mat RefereeHandsDetector::detectRedRegions(const cv::Mat& rgbFrame, const RefereeHandDetectorSettings& settings) {
   cv::Mat redMask;
   cv::Rect croppedRegion(settings.cropLeft, settings.cropTop, rgbFrame.cols - settings.cropLeft - settings.cropRight, rgbFrame.rows - settings.cropTop - settings.cropBottom);
   try {
      cv::Mat croppedFrame = rgbFrame(croppedRegion);
...
std::vector<cv::Rect> RefereeHandsDetector::detectGloveCandidates(const cv::Mat& redMask, const RefereeHandDetectorSettings& settings) {
   std::vector<cv::Rect> boxes;
   try {
      std::vector<std::vector<cv::Point>> contours;
      cv::findContours(redMask, contours, cv::RETR_TREE, cv::CHAIN_APPROX_SIMPLE);
...
std::vector<GloveCandidate> RefereeHandsDetector::detectGloves(const std::vector<cv::Rect>& boxes, const RefereeHandDetectorSettings& settings) {
   std::vector<GloveCandidate> gloveCandidates;
   for (size_t i = 0; i < boxes.size(); ++i) {
      for (size_t j = i + 1; j < boxes.size(); ++j) {
         const cv::Rect& box1 = boxes[i];
