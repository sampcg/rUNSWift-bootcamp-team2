...
# Deprecated, no longer used in Ubuntu robot OS
function setupctc() {
    ...
    for CTC_VERSION in "${CTC_VERSIONS[@]}"; do
        ...
        if [ ! -f ${LINUX_CTC_ZIP} ]; then
            ...
            if ! wget http://runswift2.cse.unsw.edu.au/opennao2/build-$CTC_VERSION/${LINUX_CTC_ZIP}; then
                echo -n 'Enter your zID (e.g. z1234567): '
                read name
                if ! wget --user=$name --ask-password robocup.web.cse.unsw.edu.au/runswift2/opennao2/build-$CTC_VERSION/ctc-linux64-atom-$CTC_VERSION.zip; then
                        rsync -aP repository@runswift2.cse.unsw.edu.au:/var/www/html/opennao2/build-$CTC_VERSION/ctc-linux64-atom-$CTC_VERSION.zip ./
                fi
            fi
        fi
        ...
        if [[ " ${CTC_VERSION} " = " ${CTC_VERSION_2_1} " ]]; then
            ...
            if [[ $(dpkg --print-architecture) == "amd64" ]]; then
                ...
                if (( $UBUNTU_VER >= 22 )); then
                    aptinstall libc6-i386 lib32z1 lib32stdc++6 lib32gcc-s1
                else
...
function setupnative() {
    ...
    aptinstall libmsgpack-dev libprotobuf-dev libbz2-dev protobuf-compiler cmake g++ libpng-dev libjpeg-dev libboost-system-dev libboost-thread-dev libboost-serialization-dev libboost-regex-dev git ccache software-properties-common
    # this is only required for utils
    sudo add-apt-repository -y ppa:ubuntuhandbook1/ppa > /dev/null
    aptinstall qt4-default libqglviewer-dev-qt5 libqwt-qt5-dev
    sudo /usr/sbin/update-ccache-symlinks
    # making sure gcc/cc points to ccache is done in setupbash
}
...
function setupdocker() {
    ...
    if [[ `id -u` -eq 0 ]]; then
        ...
        aptinstall apt-utils sudo rsync
    fi
}
...
declare -A robots
# https://runswift.readthedocs.io/en/latest/setup/nao_setup.html#robot-config-name-and-wifi-setup
#v5s
for robot in freya loki odin thor tyr vali bb8 chewie han r2d2 rey yoda pikachu mew eevee rapidash ditto abra neytiri mario; do
    robots[$robot]=2.1
done
#2018 v6s
for robot in dory nemo pearl bruce gill squirt; do
    robots[$robot]=2.8
done
#2019-05-28 v6s
for robot in frodo gandalf gimli legolas treebeard; do
    robots[$robot]=2.8
done
#2022-10-29 v6s
for robot in bondi coogee garie manly palm shelly; do
    robots[$robot]=2.8
done
# Ubuntu22 migration
for robot in aragorn; do
    robots[$robot]=2.8/ubuntu-22.04
done
