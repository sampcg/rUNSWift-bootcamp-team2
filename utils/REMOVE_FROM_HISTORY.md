...
for commit in $(sort -nr /tmp/commit-size.txt | cut -f2 -d' '); do
  if [[ "$(git merge-base $commit main)" == $commit ]]; then
...
```
git fetch origin && git checkout origin/main && for branch in $(git branch | cut -c3-); do git branch --delete --force $branch; done && git checkout master
```
