...
void LogsTab::updateDirList() {
   ...
   if (uiLogsTab.cbDirectories->currentIndex() == -1 ||
      ...
      Q_FOREACH(const QString &dir, newList)
...
void LogsTab::changeLog(const QString &logLevel) {
   vector<string> argv;
   argv.push_back("--debug.log=" + logLevel.toStdString());
   Q_EMIT sendCommandToRobot(argv);//TODO:the robot end of this
}
