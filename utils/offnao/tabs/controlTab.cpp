...
void ControlTab::controlOn() {
   ...
   if (underControl && curNao != "")
   {
      ...
      Q_EMIT showMessage("Motion Control on.");
   }
   else
   {
...
void ControlTab::drawImage(QImage *image) {
   if (currentFrame) {
      ...
      for (unsigned int row = 0; row <BOT_IMAGE_ROWS/BOT_SALIENCY_DENSITY; ++row) {
         for (unsigned int col=0; col<BOT_IMAGE_COLS/BOT_SALIENCY_DENSITY; ++col) {
...
void ControlTab::sendBehaviour()
{
   try
   {
      ...
      if (br.actions.body.actionType != ActionCommand::Body::WALK && br.actions.body.actionType != ActionCommand::Body::KICK)
         br.actions.body.actionType = ActionCommand::Body::STAND;

      if (br.actions.body.actionType != ActionCommand::Body::KICK && br.actions.head.pitchSpeed == 0.0 && br.actions.head.yawSpeed == 0.0)
         br.actions.body.actionType = ActionCommand::Body::WALK;
