   ...
   for (i = 0; i < cNUM_CAM_CONTROLS-1; i++) {
      ...
      if (strcmp(ControlNames[i],"Exposure Auto") == 0) {
        ...
        QLineEdit *currentValue =
              new QLineEdit(QString::number(controlValues[0][i]));
        ...
        slider->setSliderPosition(controlValues[0][i]);
        ...
        slider2->setSliderPosition(controlValues[0][i]);
      ...
      QLineEdit *currentValue =
            new QLineEdit(QString::number(controlValues[0][i]));
      //currentValue->setValue(controlValues[0][i]);
      QPushButton *incButton = new QPushButton("+");
      incButton->setMaximumSize(30,30);
...
void CameraTab::setControls(const vector<struct v4l2_control> &controls){
    ...
    for(vector<struct v4l2_control>::const_iterator ci = controls.begin();
       ...
       ss.push_back("--camera." + selectedCam + "." + getCameraControlName(ci->id) + "=" +
                    static_cast<ostringstream>( (ostringstream() << ci->value)).str());
    }

    Q_EMIT sendCommandToRobot(ss);
}
