...
%define NUM_COUNTERS  5              ; must match value in PMCTest.h

CounterTypesDesired:
    DD      1        ; core cycles (Intel only)
    DD      2        ; Reference clock cycles
    DD      9        ; instructions
    DD    100        ; uops
;    DD    103        ; uops from microcode rom
;    DD    111        ; any stall
;    DD    150        ; port 0 integer pipe full
;    DD    151        ; port 1 integer pipe full
    DD    152        ; port 0 f.p. pipe full
;    DD    153        ; port 1 f.p. pipe full
;    DD    158        ; memory execution cluster pipe full
;    DD    201        ; branches predicted taken
;    DD    202        ; branches not taken mispredicted 
;    DD    207        ; mispredicted branches
;    DD    209        ; early prediction corrected by later prediction
;    DD    310        ; code cache misses
;    DD    311        ; level 1 data cache load miss
;    DD    320        ; level 2 cache misses
